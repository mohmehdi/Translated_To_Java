...
public final class BasicDerAdapter<T> implements DerAdapter<T> {
    ...
    public BasicDerAdapter(String name, int tagClass, long tag, Codec<T> codec, boolean isOptional, T defaultValue, boolean typeHint) {
        ...
        if (tagClass < 0) {
            throw new IllegalArgumentException("tagClass cannot be negative");
        }
        if (tag < 0) {
            throw new IllegalArgumentException("tag cannot be negative");
        }
    }
    ...
    @Override
    public void toDer(DerWriter writer, T value) throws IOException {
        if (typeHint) {
