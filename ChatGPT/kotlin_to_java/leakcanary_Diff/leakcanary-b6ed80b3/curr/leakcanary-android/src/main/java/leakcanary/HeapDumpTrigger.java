...
public class HeapDumpTrigger {
    ...
    private void tick(String reason) {
        ...
        Config config = configProvider.invoke();
        ...
        if (retainedKeys < minLeaks) {
            CanaryLog.d(
                    "Found %d retained references, which is less than the min of %d", retainedKeys,
                    minLeaks
            );
            return;
        }
        ...
        CanaryLog.d(
                "Found %d retained references, dumping the heap", retainedKeys
        );
        ...
        HeapDump.Builder heapDump = new HeapDump.Builder()
