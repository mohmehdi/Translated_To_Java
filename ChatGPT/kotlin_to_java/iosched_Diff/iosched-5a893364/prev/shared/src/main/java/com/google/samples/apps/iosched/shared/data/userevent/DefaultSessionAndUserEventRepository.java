...
public class DefaultSessionAndUserEventRepository implements SessionAndUserEventRepository {
    ...
    private Map<ConferenceDay, List<UserSession>> mapUserDataAndSessions(UserEventsResult userData, List<Session> allSessions) {
        ...
        UserEventsResult allDataSynced = userData.getFirst();
        List<UserEvent> userEvents = userData.getSecond();
        String userEventsMessage = userData.getThird();

        Map<String, UserEvent> eventIdToUserEvent = userEvents.stream().collect(Collectors.toMap(UserEvent::getId, Function.identity()));
        List<UserSession> allUserSessions = allSessions.stream().map(session -> new UserSession(session, eventIdToUserEvent.get(session.getId()))).collect(Collectors.toList());
        ...
        return Arrays.stream(ConferenceDay.values()).collect(Collectors.toMap(day -> day, day -> allUserSessions.stream()
