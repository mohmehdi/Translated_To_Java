...
public class PlatformRule implements TestRule {
    ...
    private final java.util.List<java.util.Map.Entry<Matcher<?>, Matcher<?>>> versionChecks = new java.util.ArrayList<>();
    ...
    private void expectFailure(Matcher<? extends Object> versionMatcher, Matcher<? extends Object> failureMatcher) {
        versionChecks.add(new java.util.AbstractMap.SimpleEntry<>(versionMatcher, failureMatcher));
    }
    ...
    public void rethrowIfNotExpected(Throwable e) throws Throwable {
        for (java.util.Map.Entry<Matcher<?>, Matcher<?>> entry : versionChecks) {
            Matcher<?> versionMatcher = entry.getKey();
            Matcher<?> failureMatcher = entry.getValue();
    ...
    public static String getPlatformSystemProperty() {
        ...
        if (property == null) {
            property = switch (Platform.get()) {
                case ConscryptPlatform -> CONSCRYPT_PROPERTY;
                case OpenJSSEPlatform -> OPENJSSE_PROPERTY;
                case Jdk8WithJettyBootPlatform -> CONSCRYPT_PROPERTY;
                case Jdk9Platform -> (isCorrettoInstalled) ? CORRETTO_PROPERTY : JDK9_PROPERTY;
                default -> JDK8_PROPERTY;
            };
        }
        return property;
    }
    ...
    public static final boolean isCorrettoSupported;
    public static final boolean isCorrettoInstalled;
    ...
    static {
        ...
        if (platformSystemProperty.equals(JDK9_PROPERTY)) {
            ...
            if (Security.getProviders()[0].getName() != "Conscrypt") {
        ...
        } else if (platformSystemProperty.equals(OPENJSSE_PROPERTY) && Security.getProviders()[0].getName() != "OpenJSSE") {
        ...
        } else if (platformSystemProperty.equals(BOUNCYCASTLE_PROPERTY) && Security.getProviders()[0].getName() != "BC") {
