...
public class ScheduleTest {
    ...
    public void allDays_areClicked_showsSessions() {
        for (int i = 0; i < TimeUtils.ConferenceDays.length; i++) {
            String dayTitle = TimeUtils.ConferenceDays[i].formatMonthDay();
    ...
    public void clickFilters_showFilters() {
        ...
        onView(AllOf.allOf(withId(R.id.recyclerview), withParent(withId(R.id.filter_sheet))))
                .perform(RecyclerViewActions.scrollTo(AllOf.allOf(ScheduleFilterAdapter.FilterViewHolder.class,
        ...
        onView(AllOf.allOf(withId(R.id.filter_label), withContentDescription(checkedFilterContentDesc),
    ...
    public void filters_clearFilters() {
        ...
        onView(AllOf.allOf(withId(R.id.filter_label), withContentDescription(getActiveFilterContDesc(filter)),
    ...
    private void checkAnimationsDisabled() {
        ...
        if (scale > 0) {
            throw new Exception("Device must have animations disabled. " +
