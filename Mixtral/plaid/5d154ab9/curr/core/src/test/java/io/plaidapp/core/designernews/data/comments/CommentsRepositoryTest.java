...
import io.plaidapp.core.data.source.designernews.remote.DesignerNewsCommentsRemoteDataSource;
...
public class CommentsRepositoryTest {
  ...
  public void getComments_withSuccess() throws Exception {
    ...
    Result<List<repliesResponses>> result = new Result.Success<>(
      repliesResponses
    );
    Mockito.when(dataSource.getComments(ids)).thenReturn(result);

    Result<List<repliesResponses>> data = repository.getComments(ids);
  ...
  public void getComments_withError() throws Exception {
    List<Long> ids = List.of(1L);
    Result<List<repliesResponses>> result = new Result.Error<>(
