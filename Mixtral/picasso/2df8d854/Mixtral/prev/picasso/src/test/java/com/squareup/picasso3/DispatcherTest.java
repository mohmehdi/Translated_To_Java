...
class DispatcherTest {
  ...
  private final TestDelegatingService service = new TestDelegatingService(new PicassoExecutorService());
  ...
  public void shutdownStopsService() {
    final ExecutorService service = new PicassoExecutorService();
    dispatcher = createDispatcher(service);
    dispatcher.shutdown();
  ...
  public void performCancelDetachesRequestAndCleansUp() {
    ...
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action);
  ...
  public void performCancelMultipleRequestsDetachesOnly() {
    final Action action1 = mockAction(picasso, URI_KEY_1, URI_1);
    final Action action2 = mockAction(picasso, URI_KEY_1, URI_1);
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action1);
  ...
  public void performCancelUnqueuesAndDetachesPausedRequest() {
    final Action action = mockAction(picasso, URI_KEY_1, URI_1, mockTarget(), "tag");
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action);
    dispatcher.hunterMap.put(URI_KEY_1 + Request.KEY_SEPARATOR, hunter);
  ...
  public void performCompleteWithNoStoreMemoryPolicy() {
    final Request data = Request.Builder(URI_1).memoryPolicy(NO_STORE).build();
    final Action action = noopAction(data);
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action);
    hunter.run();
    Truth.assertThat(cache.size()).isEqualTo(0);
  ...
  public void performErrorCleansUpAndPostsToMain() {
    final Action action = mockAction(picasso, URI_KEY_1, URI_1, mockTarget());
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action);
  ...
  public void performRetryForContentLengthResetsNetworkPolicy() {
    ...
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action, e, true);
    hunter.run();
    dispatcher.performRetry(hunter);
  ...
  public void performRetryDoesNotMarkForReplayIfNotSupported() {
    final NetworkInfo networkInfo = mockNetworkInfo(true);
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), mockAction(picasso, URI_KEY_1, URI_1));
    Mockito.when(connectivityManager.getActiveNetworkInfo()).thenReturn(networkInfo);
    dispatcher.performRetry(hunter);
    Truth.assertThat(dispatcher.hunterMap).isEmpty();
    Truth.assertThat(dispatcher.failedActions).isEmpty();
  ...
  public void performRetryMarksForReplayIfSupportedScansNetworkChangesAndShouldNotRetry() {
    final NetworkInfo networkInfo = mockNetworkInfo(true);
    final Action action = mockAction(picasso, URI_KEY_1, URI_1, mockTarget());
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action, null, false, true);
    Mockito.when(connectivityManager.getActiveNetworkInfo()).thenReturn(networkInfo);
    dispatcher.performRetry(hunter);
    Truth.assertThat(dispatcher.hunterMap).isEmpty();
  ...
  public void performRetryRetriesIfNoNetworkScanning() {
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), mockAction(picasso, URI_KEY_1, URI_1), null, true);
    final Dispatcher dispatcher = createDispatcher(false);
    dispatcher.performRetry(hunter);
    Truth.assertThat(dispatcher.hunterMap).isEmpty();
    Truth.assertThat(dispatcher.failedActions).isEmpty();
  ...
  public void performRetryRetriesIfShouldRetry() {
    final Action action = mockAction(picasso, URI_KEY_1, URI_1, mockTarget());
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action, null, true);
    dispatcher.performRetry(hunter);
    Truth.assertThat(dispatcher.hunterMap).isEmpty();
    Truth.assertThat(dispatcher.failedActions).isEmpty();
    Truth.assertThat(service.submissions).isEqualTo(1);
  ...
  public void performPauseOnlyDetachesPausedRequest() {
    final Action action1 = mockAction(picasso = picasso, key = URI_KEY_1, uri = URI_1, target = mockTarget(), tag = "tag1");
    final Action action2 = mockAction(picasso = picasso, key = URI_KEY_1, uri = URI_1, target = mockTarget(), tag = "tag2");
    final Hunter hunter = mockHunter(picasso, new RequestHandler.Result.Bitmap(bitmap1, MEMORY), action1);
    hunter.attach(action2);
    dispatcher.hunterMap.put(URI_KEY_1, hunter);
  ...
  public void nullExtrasOnReceiveAirplaneDoesNothing() {
    final Dispatcher dispatcher = mock(Dispatcher.class);
    final NetworkBroadcastReceiver receiver = new NetworkBroadcastReceiver(dispatcher);
    receiver.onReceive(context, new Intent(ConnectivityManager.ACTION_AIRPLANE_MODE_CHANGED));
    Mockito.verifyZeroInteractions(dispatcher);
  }

  @Test
  public void correctExtrasOnReceiveAirplaneDispatches() throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
    setAndVerifyAirplaneMode(false);
    setAndVerifyAirplaneMode(true);
  }

  private void setAndVerifyAirplaneMode(boolean airplaneOn) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
    final Dispatcher dispatcher = mock(Dispatcher.class);
    final NetworkBroadcastReceiver receiver = new NetworkBroadcastReceiver(dispatcher);
    final Intent intent = new Intent(ConnectivityManager.ACTION_AIRPLANE_MODE_CHANGED);
    intent.putExtra(Companion.EXTRA_AIRPLANE_STATE, airplaneOn);
    final Method method = NetworkBroadcastReceiver.class.getDeclaredMethod("getConnectivityManager", Context.class);
    method.setAccessible(true);
    final ConnectivityManager connectivityManager = (ConnectivityManager) method.invoke(receiver, context);
    final ShadowConnectivityManager shadowConnectivityManager = Shadows.shadowOf(connectivityManager);
    shadowConnectivityManager.setAirplaneMode(airplaneOn);
...
public Dispatcher createDispatcher(boolean scansNetworkChanges) {
    return Dispatchers.create(service, scansNetworkChanges);
}
  ...
  private Dispatcher createDispatcher(ExecutorService service, boolean scansNetworkChanges) {
    if (scansNetworkChanges) {
      ...
      Mockito.when(connectivityManager.getActiveNetworkInfo()).thenReturn(networkInfo);
      Mockito.when(context.getSystemService(Context.CONNECTIVITY_SERVICE)).thenReturn(connectivityManager);
      Mockito.when(context.checkCallingOrSelfPermission(ArgumentMatchers.anyString())).thenReturn(PackageManager.PERMISSION_GRANTED);
      return new Dispatcher(context, service, new Handler(Looper.getMainLooper()), cache);
    } else {
      final ConnectivityManager connectivityManager = mock(ConnectivityManager.class);
      Mockito.when(context.getSystemService(Context.CONNECTIVITY_SERVICE)).thenReturn(connectivityManager);
      Mockito.when(context.checkCallingOrSelfPermission(ArgumentMatchers.anyString())).thenReturn(PackageManager.PERMISSION_DENIED);
      return new Dispatcher(context, service, new Handler(Looper.getMainLooper()), cache);
    }
  }
