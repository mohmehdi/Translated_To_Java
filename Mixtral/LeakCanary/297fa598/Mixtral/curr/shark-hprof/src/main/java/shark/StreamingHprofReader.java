...
import shark.PrimitiveType.REFERENCE_HPROF_TYPE;
...
public class StreamingHprofReader {
  ...
  private StreamingSourceProvider sourceProvider;
  private HprofHeader header;

  public StreamingHprofReader(StreamingSourceProvider sourceProvider, HprofHeader header) {
    this.sourceProvider = sourceProvider;
    this.header = header;
  }

  public long readRecords(Set < Class < ? >> recordTypes, OnHprofRecordListener listener) throws IOException {
      ...
      ObjectRecord.InstanceSkipContentRecord reusedInstanceSkipContentRecord = new ObjectRecord.InstanceSkipContentRecord(0, 0, 0);
      ObjectRecord.ClassSkipContentRecord reusedClassSkipContentRecord = new ObjectRecord.ClassSkipContentRecord(0, 0, 0, 0, 0, 0, 0, 0, 0);
      ObjectRecord.ObjectArraySkipContentRecord reusedObjectArraySkipContentRecord = new ObjectRecord.ObjectArraySkipContentRecord(0, 0, 0, 0);
      ObjectRecord.PrimitiveArraySkipContentRecord reusedPrimitiveArraySkipContentRecord = new ObjectRecord.PrimitiveArraySkipContentRecord(0, 0, 0, BOOLEAN);
      LoadClassRecord reusedLoadClassRecord = new LoadClassRecord(0, 0, 0, 0);

      return sourceProvider.openStreamingSource().use {
        source -> {
          long intByteSize = PrimitiveType.INT.byteSize;
          long identifierByteSize = header.getSizeOf(REFERENCE_HPROF_TYPE);

          HprofRecordReader reader = new HprofRecordReader(header, source);
          reader.skip(header.getRecordsPosition());
              ...
              while (reader.getBytesRead() - heapDumpStart < length) {
                ...
                case CLASS_DUMP: {
                  if (readClassDumpRecord) {
                    long recordPosition = reader.getBytesRead();
                    ObjectRecord.ClassDumpRecord record = reader.readClassDumpRecord();
                    listener.onHprofRecord(recordPosition, record);
                ...
                case INSTANCE_DUMP: {
                  if (readInstanceDumpRecord) {
                    long recordPosition = reader.getBytesRead();
                    ObjectRecord.InstanceDumpRecord record = reader.readInstanceDumpRecord();
                    listener.onHprofRecord(recordPosition, record);
                ...
                case OBJECT_ARRAY_DUMP: {
                  if (readObjectArrayDumpRecord) {
                    long recordPosition = reader.getBytesRead();
                    ObjectRecord.ObjectArrayDumpRecord arrayRecord = reader.readObjectArrayDumpRecord();
                ...
                case PRIMITIVE_ARRAY_DUMP: {
                  if (readPrimitiveArrayDumpRecord) {
                    long recordPosition = reader.getBytesRead();
                    ObjectRecord.PrimitiveArrayDumpRecord record = reader.readPrimitiveArrayDumpRecord();
                    listener.onHprofRecord(recordPosition, record);
                ...
                default:
                  throw new IllegalStateException(
                    "Unknown tag 0x" + Integer.toHexString(heapDumpTag) + " at " + heapDumpTagPosition + " after 0x" + Integer.toHexString(previousTag) + " at " + previousTagPosition);
                }
    ...
    public static final int ROOT_UNKNOWN = (byte) 0xff;
    ...
    public static StreamingHprofReader readerFor(
      File hprofFile,
    ...
    public static StreamingHprofReader readerFor(
      StreamingSourceProvider hprofSourceProvider,
      HprofHeader hprofHeader = hprofSourceProvider.openStreamingSource()
          .use(source -> HprofHeader.parseHeaderOf(source))
    ) {
