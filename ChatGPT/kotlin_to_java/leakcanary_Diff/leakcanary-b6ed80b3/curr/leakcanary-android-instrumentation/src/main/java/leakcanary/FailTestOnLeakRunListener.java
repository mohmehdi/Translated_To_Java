...
public class FailTestOnLeakRunListener extends RunListener {
    ...
    protected void reportLeaks(InstrumentationLeakResults results) {
        if (!results.detectedLeaks.isEmpty()) {
            ...
            InstrumentationRegistry.getInstrumentation().sendStatus(InstrumentationResultPrinter.REPORT_VALUE_RESULT_FAILURE, bundle);
        }
    }

    protected String buildLeakDetectedMessage(List<InstrumentationLeakResults.Result> detectedLeaks) {
