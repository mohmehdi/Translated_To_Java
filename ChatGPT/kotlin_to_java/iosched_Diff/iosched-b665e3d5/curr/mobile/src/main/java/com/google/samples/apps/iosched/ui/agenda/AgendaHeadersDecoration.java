...
public class AgendaHeadersDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = parent.getChildCount() - 1; i >= 0; i--) {
            ...
            int viewTop = view.getTop() + Math.round(ViewCompat.getTranslationY(view));
    ...
    private Map<Integer, StaticLayout> createDaySlots(List<Block> blocks) {
        ...
        for (Map.Entry<Integer, ZonedDateTime> entry : indexAgendaHeaders(blocks).entrySet()) {
            ...
            ZonedDateTime day = entry.getValue();
            daySlots.put(index, createHeader(day));
        }
        return daySlots;
    }
