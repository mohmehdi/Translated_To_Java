...
public class ExcludedRefs implements Serializable {
  ...
  public ExcludedRefs(BuilderWithParams builder) {
    ...
    public Map<String, Map<String, Exclusion>> unmodifiableRefStringMap(Map<String, Map<String, ParamsBuilder>> mapmap) {
        Map<String, Map<String, Exclusion>> fieldNameByClassName = new HashMap<>();
    ...
    public Map<String, Exclusion> unmodifiableRefMap(Map<String, ParamsBuilder> fieldBuilderMap) {
        Map<String, Exclusion> fieldMap = new HashMap<>();
  ...
  public String toString() {
    ...
    for (Map.Entry<String, Map<String, Exclusion>> entry : fieldNameByClassName.entrySet()) {
      for (Map.Entry<String, Exclusion> entry1 : entry.getValue().entrySet()) {
        ...
        string.append("| Field: ").append(entry.getKey()).append(".").append(entry1.getKey())
    ...
    for (Map.Entry<String, Map<String, Exclusion>> entry : staticFieldNameByClassName.entrySet()) {
      for (Map.Entry<String, Exclusion> entry1 : entry.getValue().entrySet()) {
        String always = entry1.getValue().alwaysExclude ? " (always)" : "";
        string.append("| Static field: ").append(entry.getKey()).append(".").append(entry1.getKey())
            .append(always).append("\n");
      }
    }
