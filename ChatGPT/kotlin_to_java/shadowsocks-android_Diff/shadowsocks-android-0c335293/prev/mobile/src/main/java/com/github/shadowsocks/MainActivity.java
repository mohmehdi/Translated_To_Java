...
public class MainActivity extends AppCompatActivity implements ShadowsocksConnection.Interface, Drawer.OnDrawerItemClickListener,
    ...
    private static ((Int) -> Unit) stateListener;
    ...
    private final int greyTint = ContextCompat.getColorStateList(this, R.color.material_primary_500);
    private final int greenTint = ContextCompat.getColorStateList(this, R.color.material_green_700);
    ...
    private void changeState(int state, String msg) {
        ...
        if (state == BaseService.CONNECTED || state == BaseService.STOPPED) {
            app.handler.postDelayed(() -> fab.setEnabled(state == BaseService.CONNECTED || state == BaseService.STOPPED), 1000);
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getResources().getDisplayMetrics().widthPixels >=
            ...
            if (getResources().getConfiguration().getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) {
                ...
                crossfader.getCrossFadeSlidingPaneLayout().setShadowDrawableLeft(
        ...
        findViewById(R.id.stat).setOnClickListener(v -> {
            if (state == BaseService.CONNECTED) {
                ++testCount;
    ...
    private void handleShareIntent(Intent intent) {
        ...
        if (!TextUtils.isEmpty(sharedStr)) {
            Profile[] profiles = Profile.findAll(sharedStr).toArray(new Profile[0]);
