...
public class PrivateDatabase extends OrmLiteSqliteOpenHelper {
    ...
    public static synchronized PrivateDatabase getInstance() {
        if (instance == null) {
    ...
    public void onUpgrade(SQLiteDatabase database, ConnectionSource connectionSource, int oldVersion, int newVersion) {
        ...
        try {
            ...
            if (oldVersion < 15) {
                ...
                long i = 0L;
                List<ApplicationInfo> apps = App.Companion.getApp().getPackageManager().getInstalledApplications(0);
                for (Profile profile : profileDao.queryForAll()) {
                    if (oldVersion < 14) {
                        Set<Integer> uidSet = Arrays.stream(profile.getIndividual().split("\\|"))
                                .filter(TextUtils::isDigitsOnly)
                                .map(Integer::parseInt)
                                .collect(Collectors.toSet());
                        profile.setIndividual(apps.stream()
                                .filter(app -> uidSet.contains(app.uid))
