...
public class MainActivity extends AppCompatActivity implements ShadowsocksConnection.Callback, OnPreferenceDataStoreChangeListener,
    ...
    private static ((BaseService.State) -> Unit) stateListener;
    ...
    public void onServiceConnected(IShadowsocksService service) {
        changeState(BaseService.State.values()[service.getState()], null);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        OnBackPressedCallback drawerHandler = new OnBackPressedCallback(drawer.isOpen()) {
            @Override
            public void handleOnBackPressed() {
                drawer.closeDrawers();
            }
        };
        drawerHandler.addOnBackPressedCallback(this);
    ...
    public boolean onKeyShortcut(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_G && event.hasModifiers(KeyEvent.META_CTRL_ON)) {
            ...
            if (toolbarFragment != null) {
                toolbarFragment.toolbar.getMenu().setQwertyMode(KeyCharacterMap.load(event.getDeviceId()).getKeyboardType() != KeyCharacterMap.NUMERIC);
                return toolbarFragment.toolbar.getMenu().performShortcut(keyCode, event, 0);
            }
            return false;
        }
    }
