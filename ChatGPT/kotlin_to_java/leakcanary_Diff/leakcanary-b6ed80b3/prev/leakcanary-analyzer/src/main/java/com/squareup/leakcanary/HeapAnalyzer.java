...
public class HeapAnalyzer {
    ...
    public HeapAnalyzer(ExcludedRefs excludedRefs, AnalyzerProgressListener listener,
        ...
        for (Class<? extends Reachability.Inspector> reachabilityInspectorClass : reachabilityInspectorClasses) {
            try {
                Reachability.Inspector inspector = reachabilityInspectorClass.getDeclaredConstructor().newInstance();
        ...
        try {
            ...
            for (Instance leakingWeakRef : leakingWeakRefs) {
                ...
                if (referent != null) {
                    ...
                    analysisResults.add(findLeakTrace(key, name, analysisStartNanoTime, snapshot, referent, computeRetainedSize, watchDurationMillis));
                } else {
    ...
    private List<Reachability> computeExpectedReachability(List<LeakTraceElement> elements) {
        ...
        for (int i = 0; i < elements.size(); i++) {
            ...
            if (reachability.getStatus() == Status.REACHABLE) {
                lastReachableElementIndex = i;
            } else if (firstUnreachableElementIndex == lastElementIndex && reachability.getStatus() == Status.UNREACHABLE) {
                firstUnreachableElementIndex = i;
            }
        }

        if (expectedReachability.get(0).getStatus() == Status.UNKNOWN) {
            expectedReachability.set(0, Reachability.reachable("it's a GC root"));
        }

        if (expectedReachability.get(lastElementIndex).getStatus() == Status.UNKNOWN) {
    ...
    private List<LeakReference> describeFields(Instance instance) {
        ...
        if (instance instanceof ClassObj) {
            ...
            for (Object key : classObj.getStaticFieldValues().keySet()) {
                ...
                leakReferences.add(new LeakReference(Type.STATIC_FIELD, name, stringValue));
            }
            ...
            if (arrayInstance.getArrayType() == Type.OBJECT) {
                ...
                for (int i = 0; i < values.length; i++) {
                    ...
                    leakReferences.add(new LeakReference(Type.ARRAY_ENTRY, name, stringValue));
                }
            }
        } else {
            ...
            for (Instance.Field field : classInstance.getValues()) {
                ...
                leakReferences.add(new LeakReference(Type.INSTANCE_FIELD, name, stringValue));
            }
        }
