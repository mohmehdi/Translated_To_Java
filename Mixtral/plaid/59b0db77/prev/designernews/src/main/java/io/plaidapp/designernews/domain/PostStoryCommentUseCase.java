...
public class PostStoryCommentUseCase {
    ...
    public Result<Comment> invoke(String body, long storyId) {
        ...
        Result<CommentWithNoReplies> result = commentsRepository.postStoryComment(body, storyId, user.getId());

        return result.exhaustive()
                .map(commentResponse -> new Result.Success<>(
                        new Comment(commentResponse, user)));
    }
}