...
public class TasksViewModel extends ViewModel {
    ...
    public void clearCompletedTasks() {
        viewModelScope.launch {
            ...
            _snackbarText.setValue(new Event<Integer>(R.string.completed_tasks_cleared));
            loadTasks(false);
        }
    }
    ...
    public void addNewTask() {
        _newTaskEvent.setValue(new Event<Unit>(Unit.INSTANCE));
    }

    public void openTask(String taskId) {
        _openTaskEvent.setValue(new Event<String>(taskId));
    }

    public void showEditResultMessage(int result) {
        switch (result) {
            case EDIT_RESULT_OK:
                _snackbarText.setValue(new Event<Integer>(R.string.successfully_saved_task_message));
                break;
            case ADD_EDIT_RESULT_OK:
                _snackbarText.setValue(new Event<Integer>(R.string.successfully_added_task_message));
                break;
            case DELETE_RESULT_OK:
                _snackbarText.setValue(new Event<Integer>(R.string.successfully_deleted_task_message));
                break;
        }
    }

    private void showSnackbarMessage(int message) {
        _snackbarText.setValue(new Event<Integer>(message));
    }
    ...
    public void loadTasks(boolean forceUpdate) {
        viewModelScope.launch {
            ...
            if (tasksResult instanceof Success) {
                ...
                List<Task> itemsValue = new ArrayList<Task>(tasksToShow);
                _items.setValue(itemsValue);
            } else {
                _dataLoading.setValue(false);
                isDataLoadingError.setValue(false);
                _items.setValue(new ArrayList<Task>());
            }
        }
    }
}
