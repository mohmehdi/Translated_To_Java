...
class InternalLeakCanary {
    ...
    static void onLeakSentryInstalled(Application application) {
        ...
        LeakCanary.ConfigProvider configProvider = () -> LeakCanary.config;
    ...
    static void onReferenceRetained() {
        if (heapDumpTrigger != null) {
            heapDumpTrigger.onReferenceRetained();
        }
    }

    static String leakInfo(
            Context context,
            HeapDump heapDump,
        ...
        if (result.leakFound) {
            ...
            if (!result.referenceName.equals("")) {
    ...
    static boolean isInAnalyzerProcess(Context context) {
        Boolean isInAnalyzerProcess = LeakCanaryInternals.isInAnalyzerProcess;

        if (isInAnalyzerProcess == null) {
            isInAnalyzerProcess = LeakCanaryInternals.isInServiceProcess(
