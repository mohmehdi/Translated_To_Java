...
public class Acl {
    ...
    public static void save(String id, Acl acl) throws IOException {
        getFile(id, App.Companion.getDeviceContext()).writeText(acl.toString());
    }
    ...
    public Acl fromReader(Reader reader) {
        return fromReader(reader, false);
    }

    public Acl fromReader(Reader reader, boolean defaultBypass) {
        clear();
    ...
    public Acl fromId(String id) {
        try {
            return fromReader(new java.io.FileReader(getFile(id, null)));
        } catch (IOException e) {
            return this;
        }
    }
    ...
    public Acl flatten(int depth) {
        if (depth > 0) {
            for (URL url : urls.asIterable()) {
                Acl child = new Acl();
                try {
                    child.fromReader(url.openStream().reader(), bypass).flatten(depth - 1);
                } catch (IOException e) {
                    continue;
                }
    ...
    public String toString() {
        ...
        result.append('\n');
        result.append(urls.asIterable().joinToString("", url -> "#IMPORT_URL <" + url + ">\n"));
