...
public class DefaultLeakDirectoryProvider implements LeakDirectoryProvider {
        ...
        if (!directoryWritableAfterMkdirs(storageDirectory)) {
            if (!hasStoragePermission()) {
                ...
                if (!Environment.MEDIA_MOUNTED.equals(state)) {
                    ...
                    Log.d(TAG, "Could not create heap dump directory in external storage: [%s]",
            ...
            if (!directoryWritableAfterMkdirs(storageDirectory)) {
                Log.d(TAG, "Could not create heap dump directory in app storage: [%s]",
                        storageDirectory.getAbsolutePath());
                return RETRY_LATER;
            }
        }
    ...
    public void clearLeakDirectory() {
        FilenameFilter nonPendingHeapDumpFilter = (dir, name) -> !name.endsWith(PENDING_HEAPDUMP_SUFFIX);
        File[] allFilesExceptPending = listFiles(nonPendingHeapDumpFilter);
    ...
    private boolean hasStoragePermission() {
        ...
        writeExternalStorageGranted = permissionCheck == PackageManager.PERMISSION_GRANTED;
    ...
    private void cleanupOldHeapDumps() {
        FilenameFilter hprofFileFilter = (dir, name) -> name.endsWith(HPROF_SUFFIX);
        File[] hprofFiles = listFiles(hprofFileFilter);
