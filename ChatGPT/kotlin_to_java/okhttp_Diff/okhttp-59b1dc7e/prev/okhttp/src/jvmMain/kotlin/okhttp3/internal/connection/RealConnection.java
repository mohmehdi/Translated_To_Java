...
public class RealConnection implements Http2Connection.Listener, Connection {
  ...
  void connect(int connectTimeout, int readTimeout, int writeTimeout, int pingIntervalMillis,
  ...
  private void startHttp2(int pingIntervalMillis) throws IOException {
    ...
    this.allocationLimit = Settings.getDefault().getMaxConcurrentStreams();
    http2Connection.start();
  }

    @Synchronized
    private void trackFailure(RealCall call, IOException e) {
  ...
  private void connectTls(ConnectionSpecSelector connectionSpecSelector) throws IOException {
    ...
    try {
      ...
      if (connectionSpec.supportsTlsExtensions()) {
        Platform.get().configureTlsExtensions(sslSocket, address.url().host(),
            address.protocols());
      }
      ...
      certificatePinner.check(address.url().host(),
          () -> {
            List<X509Certificate> certificates =
                handshake.peerCertificates().map(cert -> (X509Certificate) cert);
            return certificates;
          });
    ...
    while (true) {
      ...
      switch (response.code()) {
        ...
        case HTTP_PROXY_AUTH:
          ...
          if ("close".equalsIgnoreCase(response.header("Connection")) && tunnelRequest != null) {
  ...
  private boolean supportsUrl(HttpUrl url) {
    ...
    return !noCoalescedConnections && handshake != null
  ...
  private boolean certificateSupportHost(HttpUrl url, Handshake handshake) {
    ...
    return OkHostnameVerifier.verify(url.host(), cert);
  }
  ...
  @Override public synchronized void onSettings(Http2Connection connection, Settings settings) {
