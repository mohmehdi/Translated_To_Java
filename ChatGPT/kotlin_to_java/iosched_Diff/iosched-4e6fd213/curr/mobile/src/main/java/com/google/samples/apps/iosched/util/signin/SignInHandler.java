...
public class FirebaseAuthSignInHandler implements SignInHandler {
    ...
    @Override
    public void signIn(int resultCode, Intent data, SignInResult onComplete) {
        switch (resultCode) {
            case Activity.RESULT_OK:
                onComplete.onResult(SignInSuccess.INSTANCE);
                break;
            default:
                onComplete.onResult(new SignInFailed(IdpResponse.fromResultIntent(data).getError()));
                break;
        }
    }
