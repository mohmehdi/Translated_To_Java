...
public class ServiceNotification {
    ...
    public ServiceNotification(BaseService.Interface service, String profileName, String channel, boolean visible) {
        ...
        this.callback = new IShadowsocksServiceCallback.Stub() {
            ...
            public void trafficUpdated(int profileId, long txRate, long rxRate, long txTotal, long rxTotal) {
                service = (BaseService.Interface) service;
                Context context = (Context) service;
                String txr = context.getString(R.string.speed, TrafficMonitor.formatTraffic(txRate));
                String rxr = context.getString(R.string.speed, TrafficMonitor.formatTraffic(rxRate));
                builder.setContentText(txr + "↑\t" + rxr + "↓");
                style.bigText(context.getString(R.string.stat_summary).format(Locale.ENGLISH, txr, rxr,
    ...
    private void init() {
        ...
        update(power.isInteractive() ? Intent.ACTION_SCREEN_ON : Intent.ACTION_SCREEN_OFF, true);
