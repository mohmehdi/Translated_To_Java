...
public class DisplayLeakService extends AbstractAnalysisResultService {
    ...
    public void onHeapAnalyzed(AnalyzedHeap analyzedHeap) {
        ...
        if (resultSaved) {
                ...
                if (result.leakFound) {
                    if (result.retainedHeapSize == AnalysisResult.RETAINED_HEAP_SKIPPED) {
                        if (result.excludedLeak) {
                            ...
                            contentTitle = getString(string.leak_canary_class_has_leaked, className);
                        }
                    } else {
                        String size = Formatter.formatShortFileSize(this, result.retainedHeapSize);
                        if (result.excludedLeak) {
                            contentTitle = getString(string.leak_canary_leak_excluded_retaining, className, size);
                        } else {
                            contentTitle = getString(string.leak_canary_class_has_leaked_retaining, className, size);
                        }
                    }
                } else {
            ...
            String contentText = getString(string.leak_canary_notification_message);
            showNotification(pendingIntent, contentTitle, contentText);
        } else {
            onAnalysisResultFailure(getString(string.leak_canary_could_not_save_text));
        }
    ...
    public void onAnalysisResultFailure(String failureMessage) {
        ...
        String failureTitle = getString(string.leak_canary_result_failure_title);
