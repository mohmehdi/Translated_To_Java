...
public class TasksViewModel extends ViewModel {
    ...
    private LiveData<List<Task>> _items = _forceUpdate.switchMap(forceUpdate -> {
        if (forceUpdate) {
        ...
        return tasksRepository.observeTasks().distinctUntilChanged().switchMap(this::filterTasks);
    });
    ...
    public LiveData<Boolean> empty = Transformations.map(_items, tasks -> tasks.isEmpty());
    ...
    private LiveData<List<Task>> filterTasks(Result<List<Task>> tasksResult) {
        ...
        if (tasksResult instanceof Success) {
            ...
            viewModelScope.launch {
                result.setValue(filterItems(tasksResult.getData(), getSavedFilterType()));
            }
        } else {
    ...
    private TasksFilterType getSavedFilterType() {
        return savedStateHandle.get(TASKS_FILTER_SAVED_STATE_KEY) != null ? savedStateHandle.get(TASKS_FILTER_SAVED_STATE_KEY) : TasksFilterType.ALL_TASKS;
    }
}
