...
public abstract class AbstractAnalysisResultService extends ForegroundService {
    ...
    public AbstractAnalysisResultService() {
        super(AbstractAnalysisResultService.class.getName(), string.leak_canary_notification_reporting);
    }
    ...
    protected void onHandleIntentInForeground(Intent intent) {
        ...
        if (!intent.hasExtra(ANALYZED_HEAP_PATH_EXTRA)) {
            onAnalysisResultFailure(getString(string.leak_canary_result_failure_no_disk_space));
            return;
        }
        ...
        if (analyzedHeap == null) {
            onAnalysisResultFailure(getString(string.leak_canary_result_failure_no_file));
            return;
        }
        try {
