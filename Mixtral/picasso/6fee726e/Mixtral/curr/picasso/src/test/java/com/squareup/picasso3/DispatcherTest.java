...
public class DispatcherTest {
  ...
  private final ExecutorService executorService = spy(PicassoExecutorService.class);
  ...
  public void shutdownUnregistersReceiver() {
    dispatcher.shutdown();
    ShadowLooper.shadowLooper().idle();
  ...
  public void performCancelDetachesRequestAndCleansUp() {
    ...
    final Hunter hunter = mockHunter(picasso, RequestHandler.Result.Bitmap(bitmap1, LoadedFrom.MEMORY), action);
  ...
  public void performCancelMultipleRequestsDetachesOnly() {
    final Action action1 = mockAction(picasso, URI_KEY_1, URI_1);
    final Action action2 = mockAction(picasso, URI_KEY_1, URI_1);
    final Hunter hunter = mockHunter(picasso, RequestHandler.Result.Bitmap(bitmap1, LoadedFrom.MEMORY), action1);
  ...
  public void performCompleteWithNoStoreMemoryPolicy() {
    final Request data = Request.Builder(URI_1).memoryPolicy(MemoryPolicy.NO_STORE).build();
    final Action action = noopAction(data);
    final Hunter hunter = mockHunter(picasso, RequestHandler.Result.Bitmap(bitmap1, LoadedFrom.MEMORY), action);
    hunter.run();
    Truth.assertThat(cache.size()).isEqualTo(0);
  ...
  public void performRetryForContentLengthResetsNetworkPolicy() {
    ...
    final Hunter hunter = mockHunter(picasso, RequestHandler.Result.Bitmap(bitmap1, LoadedFrom.MEMORY), action, e, true);
    hunter.run();
    dispatcher.performRetry(hunter);
  ...
  public void performRetryDoesNotMarkForReplayIfNotSupported() {
    ...
    final Hunter hunter = mockHunter(
      picasso,
      RequestHandler.Result.Bitmap(bitmap1, LoadedFrom.MEMORY),
  ...
  public void performPauseDetachesRequestAndCancelsHunter() {
    ...
    final Hunter hunter = mockHunter(
      picasso = picasso,
      result = RequestHandler.Result.Bitmap(bitmap1, LoadedFrom.MEMORY),
      action = action,
  ...
  private Dispatcher createDispatcher(ExecutorService service, boolean scansNetworkChanges) {
    final ConnectivityManager connectivityManager = Mockito.mock(ConnectivityManager.class);
    final ShadowConnectivityManager shadowConnectivityManager = Shadows.shadowOf(connectivityManager);
    shadowConnectivityManager.setActiveNetworkInfo(scansNetworkChanges ? mockNetworkInfo() : null);
    ...
    return new HandlerDispatcher(context, service, mainHandler, cache);
  }
