...
public class DataManager implements DataLoadingSubject {
    ...
    @Inject
    public DataManager(LoadStoriesUseCase loadStories, LoadPostsUseCase loadPosts, SearchStoriesUseCase searchStories, ShotsRepository shotsRepository, SourcesRepository sourcesRepository, CoroutinesDispatcherProvider dispatcherProvider) {
    ...
    private void loadSource(SourceItem source) {
        if (source.getActive()) {
            ...
            if (source.getKey().equals(DesignerNewsSearchSourceItem.Companion.getSOURCE_DESIGNER_NEWS_POPULAR())) {
                parentJobs.put(data, launchLoadDesignerNewsStories(data));
            } else if (source.getKey().equals(DribbbleSourceItem.class)) {
    ...
    private Job launchLoadDesignerNewsStories(InFlightRequestData data) {
        return scope.launch(() -> {
            ...
            if (result instanceof Result.Success) {
                sourceLoaded(result.getData(), DesignerNewsSearchSourceItem.Companion.getSOURCE_DESIGNER_NEWS_POPULAR(), data);
    ...
    private Job loadDesignerNewsSearch(DesignerNewsSearchSourceItem sourceItem, InFlightRequestData data) {
        return scope.launch(() -> {
            Result<List<PlaidItem>> result = searchStories.invoke(sourceItem.getKey(), data.getPage());
            if (result instanceof Result.Success) {
