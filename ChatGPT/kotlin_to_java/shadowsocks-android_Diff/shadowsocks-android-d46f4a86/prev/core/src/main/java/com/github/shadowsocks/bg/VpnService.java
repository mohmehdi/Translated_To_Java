...
public class VpnService extends BaseVpnService implements LocalDnsService.Interface {
    ...
    @Override
    public InetAddress resolver(String host) {
        return DefaultNetworkListener.get().getByName(host);
    }
    ...
    private FileDescriptor startVpn() throws NullConnectionException {
        ...
        switch (profile.route) {
            ...
            default:
                ...
                for (String dnsServer : profile.remoteDns.split(",")) {
                    InetAddress address = dnsServer.trim().parseNumericAddress();
                    if (address != null) {
                        builder.addRoute(address, address.getAddress().length * 8);
                    }
                }
                break;
        }
