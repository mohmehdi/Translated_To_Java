...
class ShortestPathFinder {
    ...
    Results findPaths(
            ...
            shark.AnalyzerProgressListener listener
    ) {
        ...
        sizeOfObjectInstances = (objectClass != null) ? objectClass.readFieldsSize() : 0;
    ...
    private List<Pair<HeapObject, GcRoot>> sortedGcRoots(HeapGraph graph) {
        ...
        return graph.gcRoots.stream()
                ...
                .sorted((pair1, pair2) -> {
                    int gcRootTypeComparison = pair2.getSecond().getClass().getName().compareTo(pair1.getSecond().getClass().getName());
                    if (gcRootTypeComparison != 0) {
                        return gcRootTypeComparison;
                    } else {
                        return rootClassName.apply(pair1.getFirst()).compareTo(rootClassName.apply(pair2.getFirst()));
                    }
                })
    ...
    private void visitInstanceRecord(
        ...
        instance.instanceClass.classHierarchy.forEach(className -> {
            Map<String, ReferenceMatcher> referenceMatcherByField = fieldNameByClassName.get(className);
            if (referenceMatcherByField != null) {
                referenceMatcherByField.forEach((fieldName, referenceMatcher) -> {
                    fieldReferenceMatchers.putIfAbsent(fieldName, referenceMatcher);
                });
            }
        });

        List<FieldValue> fieldNamesAndValues = new ArrayList<>(instance.readFields());
        fieldNamesAndValues.sort(Comparator.comparing(FieldValue::getName));
...
private void enqueue(HeapGraph graph, ReferencePathNode node) {
    ...
    if (!isLeakingInstance) {
        ...
        if (heapObject instanceof HeapClass) {
            ...
            skip = graphObject.isPrimitiveWrapper || "java.lang.String".equals(graphObject.instanceClassName) || graphObject.instanceClass.instanceSize <= sizeOfObjectInstances;
        } else if (heapObject instanceof HeapObjectArray) {
            HeapObjectArray graphObject = (HeapObjectArray) heapObject;
            skip = graphObject.isPrimitiveWrapperArray;
    ...
    if (heapObject instanceof HeapClass) {
        ...
        if ("java.lang.String".equals(graphObject.instanceClassName)) {
            ...
            long valueId = graphObject.get("java.lang.String", "value") != null ? graphObject.get("java.lang.String", "value").value.asObjectId : ValueHolder.NULL_REFERENCE;
        ...
        if (graphObject.isPrimitiveWrapperArray) {
            updateDominator(parentObjectId, objectId, true);
            for (long wrapperId : graphObject.readRecord().elementIds) {
   ...
   rivate void undominateWithSkips(HeapGraph graph, long objectId) {
    HeapObject heapObject = graph.findObjectById(objectId);
    if (heapObject instanceof HeapClass) {
        undominate(objectId, false);
    } else if (heapObject instanceof HeapInstance) {
