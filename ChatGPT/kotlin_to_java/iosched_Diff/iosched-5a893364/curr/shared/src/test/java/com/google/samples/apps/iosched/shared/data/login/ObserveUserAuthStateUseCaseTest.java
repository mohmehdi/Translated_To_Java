...
public class ObserveUserAuthStateUseCaseTest {
    ...
    public void userSuccessRegistered() {
        ...
        MatcherAssert.assertThat(
                ...
                Is.is(IsEqual.equalTo(TEST_USER_ID))
        );
        MatcherAssert.assertThat(
                value.getData().isLoggedIn(),
                Is.is(IsEqual.equalTo(true))
        );
        MatcherAssert.assertThat(
    ...
    public void userErrorNotRegistered() {
        ...
        Object value = LiveDataTestUtil.getValue(subject.observe());

        MatcherAssert.assertThat(
                value,
                Is.is(Matchers.instanceOf(Result.Error.class))
        );
    }
    ...
    private ObserveUserAuthStateUseCase createObserveUserAuthStateUseCase(
            ...
            boolean isSuccess,
            String userId) {
        FakeAuthStateUserDataSource authStateUserDataSource = new FakeAuthStateUserDataSource(isAnonymous, isSuccess, userId);
        FakeRegisteredUserDataSource registeredUserDataSource = new FakeRegisteredUserDataSource(isRegistered);
