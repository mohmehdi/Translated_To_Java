...
public class StoryViewModelTest {
    ...
    private CommentsWithRepliesAndUsersUseCase commentsWithRepliesAndUsers = mock(CommentsWithRepliesAndUsersUseCase.class);
    ...
    public void loadStory_notInRepo() {
        ...
        StoryViewModel viewModel = new StoryViewModel(storyId, getStory, postStoryComment, postComment, commentsWithRepliesAndUsers, upvoteStory, upvoteComment, provideFakeCoroutinesContextProvider());
    }

    @Test
    public void commentsRequested_whenViewModelCreated() {
        StoryViewModel viewModel = withViewModel();
        StoryViewModel.UiModel event = LiveDataTestUtil.getValue(viewModel.getUiModel());
    ...
    public void upvoteStory_whenUpvoteFailed() {
        runBlocking {
            Result.Error response = Result.Error(new IOException("Error upvoting"));
    ...
    public void commentReplyRequested_withSuccess() {
        runBlocking {
            Result.Success<Comment> expected = Result.Success(reply1);
    ...
    public void commentReplyRequested_withError() {
        runBlocking {
            Result.Error expected = Result.Error(new IOException("Error"));
            whenever(postComment.invoke(reply1.getBody(), reply1.getParentCommentId())).thenReturn(expected);
            StoryViewModel viewModel = withViewModel();
            Result<Comment> result = null;
            viewModel.commentReplyRequested(reply1.getBody(), reply1.getParentCommentId(), it -> result = it);
    ...
    private StoryViewModel withViewModel() {
        ...
        runBlocking {
            whenever(commentsWithRepliesAndUsers.invoke(commentIds)).thenReturn(Result.Success(flattendCommentsWithReplies));
        }
        return new StoryViewModel(storyId, getStory, postStoryComment, postComment, commentsWithRepliesAndUsers, upvoteStory, upvoteComment, provideFakeCoroutinesContextProvider());
    }
}